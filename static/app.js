...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (rateSlider  && rateInput)   rateSlider.addEventListener('input',  () => { rateInput.value   = rateSlider.value; });
  if (rateInput   && rateSlider)  rateInput.addEventListener('input',   () => { rateSlider.value  = rateInput.value; });
  if (tenureSlider && tenureInput) tenureSlider.addEventListener('input', () => { tenureInput.value = tenureSlider.value; });
  if (tenureInput  && tenureSlider) tenureInput.addEventListener('input', () => { tenureSlider.value = tenureInput.value; });
  ...
  //  AREA FINDER
  // ============================================================
  const areasForm = document.getElementById('areasForm');
  if (areasForm) {
    areasForm.addEventListener('submit', async (e) => {
      e.preventDefault();
  ...
  async function sendChatMessage(message) {
    if (!message || !message.trim() || !chatInput || !chatSendBtn) return;
    chatInput.value = '';
    ...
    try {
      ...
      if (typingEl && chatMessages && chatMessages.contains(typingEl)) chatMessages.removeChild(typingEl);
  ...
  if (chatSendBtn) chatSendBtn.addEventListener('click', () => sendChatMessage(chatInput ? chatInput.value : ''));
